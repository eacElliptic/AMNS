...
void init_data(){
	...
	mpz_set_str (modul_p, "5889103864257702068504487044228158737536277610392921143852982809793938105170375651823445397569691387467898043325195002746366416757682868551262886674213011557", 10);

	mpz_set_str (gama_pow[0], "4759800655156854818722220631793485478038952247599601811114583588528085057392565748647136408480719818940287088791074358821752751850817178119353218893423283016", 10);
	for(i=1; i<POLY_DEG; i++){
		mpz_mul (gama_pow[i], gama_pow[i-1], gama_pow[0]);
		mpz_mod (gama_pow[i], gama_pow[i], modul_p);
	}

	//~ Note : lambda = -4 (see : modular multiplication in 'add_mult_poly.c').
