...
void init_data(){
	...
	mpz_set_str (gama_pow[0], "1117153186288219732686123191778928509368377420875867732372644745599022035665037995983537131161653843295860258283071696235760737034275476252735648017849930298", 10);
	for(i=1; i<POLY_DEG; i++){
		mpz_mul (gama_pow[i], gama_pow[i-1], gama_pow[0]);
		mpz_mod (gama_pow[i], gama_pow[i], modul_p);
	}

	//~ Note : lambda = -9 (see : modular multiplication in 'add_mult_poly.c').
